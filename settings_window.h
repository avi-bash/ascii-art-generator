...
public:
    explicit SettingsWindow(RenderSettings& settings) : settings_(settings) {
        ...
        cv::resizeWindow(windowName_, 620, 960);
        ...
        if (settingsHwnd_ != nullptr) MoveWindow(settingsHwnd_, 100, 100, 620, 960, TRUE);
        drawCheckboxes();
    }
...
private:
    ...
    void drawCheckboxes() {
        ...
        const int height = std::max(960, imageRect.height);
        const double uiScale = std::min(width / 620.0, height / 960.0);
    ...
    double scale() const {
        ...
        return std::min(std::max(620, imageRect.width) / 620.0,
            std::max(960, imageRect.height) / 960.0);
    }
