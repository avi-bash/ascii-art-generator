...
int main(int argc, char** argv) {
    ...
    if (argc == 5) {
        ...
        std::cerr << "Usage: ascii-translation-gpu.exe [video-path] [red] [green] [blue] [-t|-o] [--transparent-threshold|-tt value] [--glow|-g] [--glow-strength|-gs value] [--glow-falloff|-gf value] [--glow-resolution|-gr value] [--glow-radius|-grd value]" << std::endl;
        return -1;
    }
    ...
    while (true) {
        ...
        if (enableGlow) {
            const cv::Size glowSize(
                std::max(1, static_cast<int>(outputWidth * glowResolution)),
                std::max(1, static_cast<int>(outputHeight * glowResolution)));
            ...
            const cv::Size blurKernel = blurRadius > 0
                ...
                : cv::Size(0, 0);
            cv::GaussianBlur(gpuGlowSmall, gpuGlowSmall, blurKernel, glowFalloff / glowResolution);
